...
if not hasattr(torch, '_original_no_grad_saprot'):
    ...
    def patched_no_grad(*args, **kwargs):
        ...
        if len(args) == 1 and callable(args[0]) and not kwargs:
            # 如果直接传入函数，说明是装饰器模式 @torch.no_grad
            func = args[0]
            def wrapper(*func_args, **func_kwargs):
                with torch._original_no_grad_saprot():
...
import sys
sys.path.insert(0, f'{root_dir}/SaprotHub/ColabESM3')
try:
    from esm3_lora_utils import (
        LoRALinear,
        ESM3LoRAWrapper,
        create_esm3_lora_model,
        fix_lora_parameters,
        load_esm3_with_lora
    )
    LORA_AVAILABLE = True
...
def is_lora_model(model):
    ...
    """
    return isinstance(model, ESM3LoRAWrapper) if LORA_AVAILABLE else False
...
def print_lora_info(lora_model):
    ...
    print("=" * 60)
    print(f"LoRA Rank: {lora_model.r}")
    print(f"LoRA Alpha: {lora_model.alpha}")
    print(f"LoRA Dropout: {lora_model.dropout}")
    print(f"Target Modules: {lora_model.target_modules}")
    print(f"Number of LoRA Layers: {len(lora_model.lora_layers)}")
...
def load_embedding_generation_model(model_type, model_arg):
  ...
  if model_type in ["Official ESM3 (1.4B)"]:
...
def generate_embeddings(protein_list, model_type, model_arg):
  ...
  if isinstance(protein_list, str) and protein_list.endswith('.fasta'):
    # 处理fasta文件中的序列
    sequences = []
...
# 暂时注释掉 obtain_protein_embedding 函数
"""
...
def encode_sequence_only(model, sequence, device):
    ...
    with torch.no_grad():
        ...
        for attr_name in ['sequence', 'structure', 'coordinates']:
            if hasattr(output_seq, attr_name):
                attr_value = getattr(output_seq, attr_name)
...
def encode_sequence_and_structure(model, sequence, coordinates, device):
    ...
    with torch.no_grad():
        ...
        result = {}
        for attr_name in ['sequence', 'structure', 'coordinates']:
            if hasattr(output_struct, attr_name):
                attr_value = getattr(output_struct, attr_name)
                if attr_value is not None:
                    result[attr_name] = attr_value.cpu().numpy()
