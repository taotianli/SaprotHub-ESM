...
class SaprotPairRegressionDataset(LMDBDataset):
    ...
    def __getitem__(self, index):
            ...
            if self.esm_model is not None:
                ...
                with torch.no_grad():  # 编码时不需要梯度
                    try:
                        ...
                        # print(f"[pair回归数据集调试] 索引 {index} - ESM3编码失败: {str(encode_error)}")
                        # 发生错误时返回原始序列
                        sequence_embedding_1 = seq_1
                        sequence_embedding_2 = seq_2
            else:
        ...
        return sequence_embedding_1, sequence_embedding_2, entry["label"]
    ...
    def collate_fn(self, batch):
        ...
        label_ids = torch.tensor(label_ids, device=self.model_device)
        ...
        if torch.is_tensor(first_embedding_1) and torch.is_tensor(first_embedding_2):
            ...
            # print(f"[pair回归数据集调试] 批处理大小: {len(embeddings_1)}, 固定token长度: {first_embedding_1.shape}")
            ...
            # 使用传统tokenizer处理
            # print(f"[pair回归数据集调试] 使用传统tokenizer处理批处理")
            ...
            # 包含原始序列（编码失败的情况）
            # print(f"[pair回归数据集调试] 批处理包含原始序列，将由模型处理")
